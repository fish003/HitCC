...
- 本页适用于 `docs/` 下所有主题文档。
- 若现有页面与本页职责边界冲突，以本页为准，并在后续整理时逐步回收混合写法。

## 当前状态

- 本页是知识库维护约定，不承载当前 bundle 运行事实。
- 结构判断以当前 `docs/` 目录、主题矩阵、evidence map 和已经拆分后的父页/子页职责为准。
- 新增证据、压缩名校准和运行时边界应写入对应主题页；本页只维护页面类型、拆分规则和文风边界。
...
后续若继续压缩父页，应优先巡检已经标为导航页但仍可能承载正文判断的页面，例如：

- `02-execution/05-attachments-and-context-modifiers.md`
- `03-ecosystem/07-tui-system.md`

这类页面只在发现重复正文、证据落点或未决项散落时继续收缩；单纯因为篇幅较长，不应删除仍有独立职责的信息。
