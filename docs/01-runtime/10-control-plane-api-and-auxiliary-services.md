...
- 当前 `2.1.197` 的直接锚点来自 `Ns()` host/OAuth 配置、`f8(...)` first-party provider client、`uH(...)` org-scoped remote header、`WBp() / N2a(...)` remote managed settings、`Hoe(...) / Smt(...) / _q(...)` remote environment/session 主链，以及 bridge 侧 session/event helpers。
...
- `api.anthropic.com`、`platform.claude.com`、`claude.ai / claude.com` 相关 OAuth/web host、`mcp-proxy.anthropic.com`
...
- `/v1/sessions*` REST、managed-agents SDK session endpoints 与 `/v1/code/sessions/{id}/events/stream` SSE 主链
