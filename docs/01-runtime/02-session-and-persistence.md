...
- `Zn()` -> `CLAUDE_CONFIG_DIR ?? ~/.claude`，并做 NFC 归一化
- `t1()` / `SF()` -> `join(Zn(), "projects")`
- `Sj(projectPath)` -> `join(t1(), normalizeProjectId(projectPath))`
- `Zf()` -> 当前 project/session 的 `<session-id>.jsonl`
- `Tk(agentId)` -> 当前 session 目录下 `subagents/agent-<id>.jsonl`
...
当前 resume/read-session 入口仍是统一“读会话”模型，支持：
