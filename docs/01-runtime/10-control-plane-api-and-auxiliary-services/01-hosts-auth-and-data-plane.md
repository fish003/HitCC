...
remote / environment / session control plane
  ...
  -> /v1/code/sessions/{id}/events/stream
...
- 当前 host 与 auth 结论来自 `Ns()`、`f8(...)`、`uH(...)`、`WBp() / N2a(...)`、`Hoe(...) / Smt(...) / _q(...)` 等当前抽取 bundle 入口，而不是旧版 host 表。
...
- `claude.com`
  - `CLAUDE_AI_AUTHORIZE_URL` 相关 OAuth authorize host
...
不能把 first-party web/auth host 简化成只剩 `platform.claude.com`。
...
first-party 数据面鉴权主要由 provider factory `f8(...)` 交给 SDK client 处理：

- subscriber 路径：`authToken = OAuth access token`，最终表现为 `Authorization: Bearer <oauth access token>`
- API customer 路径：`apiKey = resolvedApiKey`，最终表现为 `x-api-key: <key>`
- 非 subscriber 路径还可能把 `ANTHROPIC_AUTH_TOKEN` 或 `apiKeyHelper` 结果注入为 header 级 `Authorization: Bearer ...`
...
`claudeai-proxy` transport 不直接复用 provider client 的数据面鉴权，而是：
...
- 调用入口：`client.beta.messages.create(...)`，上层来自 `Hpc(...) / HSt(...)`
- 认证：`f8(...)` 构造出的 SDK client auth，即 subscriber bearer 或 API key / external bearer 组合
- 与 session/org 的关系：
