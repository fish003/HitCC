...
- 调用入口：`SNp()`；web-setup UI 侧还有 `hac()` 走 `ks.get("/api/oauth/organizations/:orgUUID/sync/github/auth", { auth: "teleport-org" })`
- 认证：Bearer + `x-organization-uuid`
...
- 认证：
  - 优先走 first-party bearer / api key auth selector
...
`ZZ1.transformLogsToEvents(...)` 会把 OTEL log record 序列化成两类 payload，见 `package/preprocessed/cli.extracted.bundle.pretty.js`。
