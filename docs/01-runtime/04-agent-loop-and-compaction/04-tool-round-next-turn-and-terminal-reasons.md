...
当前主循环会创建 streaming tool runner：

- `new KHe(tools, canUseTool, toolUseContext, now)`
- streaming 过程中先通过 `getCompletedResults()` 抽取已经完成的结果
- model stream 结束后再通过 `getRemainingResults()` drain 剩余工具结果
...
工具阶段结束后，`oRf(...)` 还会顺次补这些东西进工具结果链：

- `KE6(...)` 产出的 attachment / queued command / context layer 相关物化消息
