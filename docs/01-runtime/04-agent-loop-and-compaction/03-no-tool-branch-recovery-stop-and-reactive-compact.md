...
- `oRf(...)` 在 reactive/precomputed compact 成功后，会先 `yield* FOo(result)`，再用 `SHe(result)` 重建下一轮消息链。
- 手动 compact UI 的 reactive-only 路径 `V7z(...)` 也把 `$.result` 直接包装成普通 `compactionResult` 返回。
- 说明 reactive compact 的成功产物，至少在公共接口上必须满足：
  - **`FOo(...) / SHe(...)`-compatible compactionResult**

但继续向内追时，出现了一个更强的静态信号。

#### 主循环里的 `J26` 当前是未接线槽位

对本地抽取 bundle 做精确搜索后，`J26` 只出现 8 次：
...
- 从接口设计看，reactive compact 若被接上，成功产物必须满足 `FOo(...) / SHe(...)` 公共合同
- 但在**当前发行版 bundle** 里：
...
当前 stop hook 子状态机不是简单 bool 检查，而是一条会自己 `yield` 消息的路径。
...
- `preventContinuation = true`
  - `oRf(...)` 直接返回 `{ reason: "stop_hook_prevented" }`
...
对本地抽取 bundle 做精确搜索后，没有看到除此之外的其他读点。  
所以当前可以把结论再收紧一层：
