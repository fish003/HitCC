...
- 用来单独记录 `_3(...)` 在进入 `zN(...) / oRf(...)` 前做的裁剪、override 与 `SubagentStart` hook 注入顺序。
...
- 主线程普通 query、`_3(...)` 驱动的普通 subagent/sidechain、以及 `vk(...)` 驱动的 fork-family，请求级别最终仍复用同一套 `zN(...) / oRf(...) -> Hpc(...) -> client.beta.messages.create(...)` 骨架。
- 真正的差异主要出现在进入 `zN(...) / oRf(...)` 前的输入裁剪与 override，而不是另起一套 merge 协议。

## 非主线程路径：前两类

### 非主线程路径：现在可以拆成三类

把 `_3(...)`、`vk(...)`、`zN(...) / oRf(...)`、`hook_agent`、`compact` 几条线对完后，当前更稳的结论不应再写成“非主线程大体复用主线程 merge”。
...
```text
zN(...) / oRf(...)
  -> system sections = CDl(systemPromptSections, systemContext)
  -> messages = otr(normalizedMessages, userContext)
...
- 主线程普通 query
- `_3(...)` 驱动的普通 subagent / sidechain
- `vk(...)` 驱动的 fork-family 请求

其中 `vk(...)` 已可视为一个“fork runner”：
...
- `side_question`
- `speculation`
- 以及其他通过 `vk(...)` 发起的 side query

### 第二类：共享 merge 骨架，但会裁剪或 override 输入

这一类**不是换了 merge 算法**，而是在进入 `zN(...) / oRf(...)` 前先改 `systemPrompt / userContext / systemContext / messages`。
...
base:
  ...
  systemPrompt = override.systemPrompt ?? Ec(await ex_(...))
...
then:
  zN({ messages, systemPrompt, userContext, systemContext, ... })
```

所以现在已经能钉死：

- `omitClaudeMd` 不是“换成另一套 merge”
- 它只是**在 `userContext` 层删掉 `claudeMd` 后，仍走同一个 `otr(...)`**
- `Explore / Plan` 不是“没有 systemContext”
- 而是**裁掉 `gitStatus` 后，仍走同一个 `CDl(...)`**
...
- `fork`
  ...
  - 但 `userContext/systemContext` 仍回到 `_3(...)` 这套装配/裁剪逻辑
- `magic_docs`
  - 直接把父 query 的 `systemPrompt / userContext / systemContext` 整包 override 给 `_3(...)`
...
- 然后才执行 `Mzt(agentId, agentType, signal)` 这条 `SubagentStart` hook runner
- `Mzt(...)` 返回的 `additionalContexts` 会先累积进 `z6[]`
- `z6[]` 非空时，`_3(...)` 才会额外构造一个：
...
forkContextMessages
  ...
  -> zN(...) / oRf(...)
  -> Hpc(...)
  -> otr(F, userContext) 前插到最前
  -> H.callModel({ messages: otr(F, userContext), systemPrompt: Wlm(CDl(...)) })
```
...
- `hook_additional_context` 作为 attachment 进入 `Ak(...)` 后，会被 `dt1(...)` 物化成 meta user message
- 若它左侧已经是 user message，`Ak(...) / Qcm(...)/rdr(...)` 可能把它继续并进同一个 user message
- 所以“位于末尾”说的是 **进入 `zN(...) / oRf(...)` 前的 transcript entry 顺序**
...
- `_3(...)` 对 `Mzt(...)` 的消费又只是在 `for await` 里按到达顺序 `push(...additionalContexts)`
- 因而 **同一事件下多个 hook 的 `additionalContext` 最终顺序是完成顺序，不是 matcher/配置顺序**

还有一个容易误判的边界：

- `Mzt(...)` 底层仍能产出 `blockingError`、`preventContinuation`、`message` 等通用 hook 结果
- 但 `_3(...)` 这个调用点当前只消费 `additionalContexts`
