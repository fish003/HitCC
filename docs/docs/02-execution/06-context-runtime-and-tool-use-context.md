...
- 用来减少后续继续把 `contextLayers`、sidechain 继承规则、fork/subagent 的上下文裁剪混写到其他页面。
...
已直接确认由 `hS()` 生成，当前本地 bundle 的默认字段只有：
...
它不是最终 request 的 `system` 字段，而是会被 `otr(...)` 包成前置 `<system-reminder>` user meta message。

当前 `2.1.197` 中，`currentDate` 由 `qla(GSe())` 生成。非 first-party `ANTHROPIC_BASE_URL` 会进入 host / timezone 标记逻辑：host 的 domain-list 命中状态、keyword 命中状态决定撇号字符，`Asia/Shanghai / Asia/Urumqi` 决定日期分隔符是否从 `-` 改成 `/`。详细链路见 [02-instruction-discovery-and-rules/04-user-system-context-and-claude-md-boundaries.md](./02-instruction-discovery-and-rules/04-user-system-context-and-claude-md-boundaries.md) 的 `currentDate` 小节。

### 3. request-level `systemContext`

已直接确认由 `_H(...)` 生成，当前本地 bundle 的默认字段只有：
...
它会被 `CDl(...)` 通过 `Object.entries(systemContext)` 序列化后，追加到 system prompt sections 末尾。
...
## `_3(...)`：subagent/sidechain 的 request context 继承规则

`_3(...)` 这条链同时处理 request-level context 和 tool-level context。
...
`_3(...)` 里对下列状态的变更，是通过包装后的 `getAppState()` 暴露给后续执行链，而不是直接改全局 store：
...
这点和 SkillTool / fork skill 的 context overlay 很像：都是**返回或预置一组会影响后续 `ToolUseContext` 的 layer**，而不是立即硬改外层 app state。

## `contextLayers` 与 `ToolUseContext` 的边界

`contextModifier` 是本组文档早期使用的概念名；当前 `2.1.197` bundle 的活实现应按 `contextLayers / permissionLayers / NYt(...)` 理解。接口、consumer、并发提交规则，以及当前本地 bundle 内已确认的 concrete producer，已经收敛到 [05-attachments-and-context-modifiers/03-context-modifier-and-executor-consumers.md](./05-attachments-and-context-modifiers/03-context-modifier-and-executor-consumers.md)。

本页只保留和 context runtime 直接相关的结论：

- `contextLayers` 不是 UI 附件，而是可枚举的 `ToolUseContext` overlay 协议
- 执行器会把它应用到共享上下文，因此下一工具调用、下一 side-path、以及部分 fork/subagent helper 看到的是改写后的 `ToolUseContext`
- 当前本地 bundle 中，tool-returned concrete producer 已正证到 `SkillTool` 与 `EnterWorktree`
- fork skill / fork slash command 会在进入 `_3(...)` 前预置 `permissionLayers`，不是父线程 tool result 返回后再提交
- remote / bridge 的状态写入则是旁路缓存更新或入口 overlay，不是同一套 tool-returned 协议

## skill discovery trigger 的当前边界

当前 2.1.197 bundle 中，`ToolUseContext` 仍保留 `dynamicSkillDirTriggers` 作为动态 skill attachment 的本轮触发器：

- 文件读写路径通过 `d_t(paths, cwd)` 找到相关 `.claude/skills` 目录
- 目录会进入 `dynamicSkillDirTriggers`
- `p_t(dirs)` 会把新 skill 装进运行时 `dynamicSkills`
- attachment 阶段由 `sDf(ctx)` 产出 `dynamic_skill`，随后清空本轮 trigger

旧文档中的 `discoveredSkillNames` 在当前 bundle 中没有活字面命中；`skill_listing` 的增量发送由 `Z7t / cMl(...)` 控制，不应再把 `discoveredSkillNames` 当成本地确定字段。
...
request-level context
  userContext = hS() = { claudeMd?, userEmail?, attachedProject?, currentDate }
  systemContext = _H(...) = { gitStatus?, perforceMode? }
  -> consumed by `CDl(...) / otr(...)` during `Hpc(...)` request assembly
...
- `systemContext` 的本地来源只看到：
  - `_H(...)` 生成的 `{ gitStatus?, perforceMode? }`
  - 显式 override 透传
  - 个别专用路径传入的空对象 `{}`

### `_H(...)` 预留第二槽位：在 Context 页里的更稳解释

就 Context 这一页的职责边界，可以把 `_H(...)` 里的第二槽位收紧成下面三件事：

1. `systemContext` 的序列化器 `CDl(...)` 是泛型 `Object.entries(...)`
2. `_H(...)` 的完成日志仍保留 `has_injection`
3. 但当前本地 `_H(...)` 实现里，入参只进入 memo key 与 `has_injection` telemetry；返回对象里对应 spread 仍为空

因此：

- 这个槽位说明 `systemContext` 设计上允许后续再扩字段
- 但当前本地 bundle 可执行路径里，除了 `gitStatus? / perforceMode?`，**没有看到第二个注入槽位写入 request-level `systemContext`**
- 在 Context 视角下，更合理的写法是“预留能力仍在，活数据流暂时不存在”，而不是“本地还有一个没找全的当前字段”
- 还能再补一条很关键的负面证据：
  - `_3(...)` 对 `Explore / Plan` 的通用裁剪只显式移除 `gitStatus`
  - 写法是 `{ gitStatus, ...rest } = systemContext`，随后仅在这两类 agent 上把 `rest` 继续传下去
  - 这意味着 `perforceMode` 或未来第二槽位如果存在，都会随 `rest` 继续传下去
  - 因此第二槽位看起来也不像“另一个与 gitStatus 同类、专门给 Explore/Plan 一起屏蔽的字段”
...
- 但 request-level context 仍由本地 `hS() / _H(...) / systemPrompt selection / Hpc(...)` 这一套生成
...
- 远端/服务端侧是否存在本地 bundle 未暴露的第二个 `contextLayers` producer
...
3. `contextLayers` 的核心价值，不是展示，而是把工具结果转成下一步运行态
