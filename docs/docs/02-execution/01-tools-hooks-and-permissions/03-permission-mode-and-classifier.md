...
2. skill `contextLayers` / `permissionLayers` 带来的 allow overlay
...
当前 2.1.197 还应补上 `Tool(param:value)` 这一类更显式的参数匹配合同。内嵌提示里已经把例子写成：

- `deny Agent(model:opus)`
- `ask Bash(run_in_background:true)`

这说明 permission rule 的 matcher 不只支持 shell 子命令、路径和 domain，也能按 tool input 的具体参数值做规则匹配。恢复实现时要保留两个边界：

- 它仍然是 permission rule syntax，不是 hook JSON schema。
- 参数匹配只能作为规则判定的一层，不能绕过 tool 自己的 `checkPermissions(...)`、PreToolUse hook、mode 状态机和最终 prompt backend。
...
当前 2.1.197 还新增了更明确的 hard-deny 线路：

- schema 中可见 `autoMode.hard_deny`
- 聚合链会把各来源的 `hard_deny` 合并后传给 classifier prompt
- `classifyAllShell` 会跨 settings source 检查；任一来源设置为 true，就把 shell 类动作纳入 classifier 覆盖面

证据点是 `package/preprocessed/cli.extracted.bundle.pretty.js:65364-65384` 与 `68901-68918`。因此当前更准确的写法是：`deny` 这个旧字段仍不能写成已消费；`hard_deny` 与 `classifyAllShell` 则是本地可见的当前配置面。
...
PowerShell / Bash 当前防线也应按现实现成机制写，不复述历史漏洞：

- 过宽 `Bash(...)` / `PowerShell(...)` allow rule 会在 auto mode 下被主动剥离，避免 auto mode 继承危险规则。
- PowerShell 单引号 literal 构造会拒绝 U+2018..U+201F 这类 quote-variant codepoint，避免 tokenizer 视角和本地 quote helper 视角不一致。
- Windows sandbox shell / filesystem deny 仍会在 sandbox runtime 层继续收口；这和 classifier 是否允许不是同一层。

这些是当前防线，不等价于“所有 shell 都能自动安全分类”。当 `classifyAllShell` 未开启或 fast-path 判定要求人工审批时，仍应保留 ask / deny 结果。
