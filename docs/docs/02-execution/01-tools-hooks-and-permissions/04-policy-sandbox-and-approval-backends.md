...
#### 2.1.197 可见 sandbox key 表

当前 settings schema 与 runtime consumer 可以把 sandbox key 收成下表，证据主要在 `package/preprocessed/cli.extracted.bundle.pretty.js:63000-63180`、`66882-66905`、`251000-252922`。

| key | 平台 / 来源边界 | 运行语义 |
| --- | --- | --- |
| `sandbox.network.deniedDomains` | 所有 settings source 合并；支持与 allowedDomains 同类 wildcard | 永远阻断匹配域名，即使 allow side 命中 |
| `sandbox.network.allowManagedDomainsOnly` | managed settings 才有收缩语义 | allow side 只保留 managed `allowedDomains` 与 managed `WebFetch(domain:...)` allow；deny side 仍合并所有来源 |
| `sandbox.credentials.files[]` | 路径按 settings 文件根、`~` 或绝对路径解析 | `mode: "deny"` 阻断 sandbox 内读取文件或目录；无效项会被忽略并报告，修好前不提供保护 |
| `sandbox.credentials.envVars[]` | env 名必须是合法 identifier | `mode: "deny"` 会在 sandboxed command 环境中 unset 对应变量 |
| `sandbox.allowAppleEvents` | macOS only；仅 user、managed/policy 或 CLI `--settings` 生效，project settings 被忽略 | 允许 Apple Events / `appleeventsd` lookup；会削弱 code-execution isolation |
| `sandbox.bwrapPath` | Linux/WSL only；只接受绝对路径；只 honored from admin-controlled managed settings | 覆盖 bubblewrap binary autodetect |
| `sandbox.socatPath` | Linux/WSL only；只接受绝对路径；只 honored from admin-controlled managed settings | 覆盖 sandbox network proxy 使用的 `socat` binary |
| `sandbox.allowUnsandboxedCommands` | runtime sandbox policy | 为 false 时忽略工具请求的 unsandboxed fallback，强制走 sandbox |

### `apply-seccomp` 是 sandbox helper，不是权限规则

Linux sandbox 还有一条 `apply-seccomp` helper 查找链：

- 若 `seccompConfig.applyPath` 显式给定且文件存在，优先使用该路径
- 否则按当前架构查找 embedded `apply-seccomp`
- 再尝试全局安装位置下的 `vendor/seccomp/<arch>/apply-seccomp`
- 找不到时会降级为 `seccomp not available - unix socket access not restricted`

因此 `apply-seccomp` 的恢复位置应放在 sandbox runtime helper 层，而不是 permission rule / approval backend 层。它影响的是 Linux sandbox 对 unix socket 访问的限制能力；即使缺失，也不代表普通 allow/deny/ask 规则失效。
...
当前本地证据可以把“是否还有其他本地 classifier 入口”收紧到一个主调用点。
