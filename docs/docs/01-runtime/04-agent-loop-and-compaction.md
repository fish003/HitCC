...
### 压缩链 / compact producer / `compactTracking`
...
- `microcompact` 与 `deps.autocompact(...)`
- `compactionResult -> FOo(...) / SHe(...)` 合同
- `compact_boundary`、`preservedSegment`
- full / partial / session-memory compact 差异
- `compactTracking` 的熔断与 telemetry 语义
...
- stop hook 子状态机与 `stopHookActive`
...
- `KHe` streaming tool runner 与工具结果 drain
...
1. 先看 [01-main-loop-state-caches-and-yield-surface.md](./04-agent-loop-and-compaction/01-main-loop-state-caches-and-yield-surface.md)，建立 `oRf(...)` 的状态机骨架和对外事件面。
