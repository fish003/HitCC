...
- policy limits
  ...
  - 产物是 `restrictions / compliance_taints / monitoring_notice / defaults`
...
### `compliance_taints` 是 policy limits 的另一层 deny 输入

`policy limits` 响应 schema 不只包含逐项 `restrictions`。当前本地还会解析：

- `compliance_taints`
- `monitoring_notice`
- `defaults`

其中 `compliance_taints` 会在 `isPolicyAllowed(...)` 里参与判定：如果某个 capability 没有显式 restriction，仍可能因为 taint 映射被拒绝。当前能直接看到的 taint 映射包括：
...
`monitoring_notice` 则会进入本地通知状态，文本会去掉控制字符，URL 要求是 `https://`。这说明 policy limits 同时承担 capability gate、合规 taint 与组织监控提示三类职责；但这些仍是显式远端 policy 机制，不是 prompt 内隐藏标记。
