...
### 2.1 外围合同层的 settings/env key 边界

当前 2.1.197 bundle 里还能补出一组外围合同 key。它们不适合写成全量 schema dump，但会影响重写兼容、host integration、TUI 行为或排障判断。

| key / env | 当前证据层级 | 消费边界 |
| --- | --- | --- |
| `parentSettingsBehavior` | settings schema 与 merge helper 均可见 | 只控制 SDK parent tier / `--managed-settings` 是否能在 admin tier 之下以 restrictive-only 方式合并；没有 admin tier 时，parent 自己仍可作为 policy tier |
| `attribution.sessionUrl` | `settings.attribution.sessionUrl` schema 可见 | 控制 web / Remote Control 创建的 commit 与 PR 是否追加 Claude session link；它是 attribution 输出合同，不是 session persistence 开关 |
| `worktree.bgIsolation` | `settings.worktree.bgIsolation` schema 与 session state 字段可见 | 控制 background session 默认是否用 worktree 隔离；`none` 允许后台任务直接编辑当前 working copy |
| `footerLinksRegexes` | settings schema 与 footer badge 消费点可见 | 只从 user / flag / managed settings 读取，忽略 project 和 local settings；最多渲染 5 个 badge，`/clear` 会清掉 |
| `autoScrollEnabled` | settings schema 与 TUI 消费点可见 | fullscreen renderer 的自动滚到底部开关，不是 classic renderer 的通用滚动策略 |
| `wheelScrollAccelerationEnabled` | settings schema 与 TUI 消费点可见 | fullscreen renderer 下鼠标滚轮快速滚动加速，和 `/scroll-speed` 的每格行数设置是两层不同控制 |
| `CLAUDE_CLIENT_PRESENCE_FILE` | env export 与消费点可见 | client presence 文件路径；属于 host / remote presence surface，不是普通 settings key |
| `ANTHROPIC_WORKSPACE_ID` | provider/client metadata 读取可见 | 写入 workspace attribution / metadata 侧；不能据此推出组织策略下发内容 |
| `CLAUDE_PROJECT_DIR` | prompt / MCP 环境替换点可见 | 作为项目目录占位替换给 MCP / prompt 侧使用，不是 `cwd` 的唯一来源 |
| `CLAUDE_CODE_DISABLE_ALTERNATE_SCREEN` | env export 与 TUI 分支可见 | changelog 中的 `DISABLE_ALTERNATE_SCREEN` 在当前 bundle 应按这个完整 env 名写；作用是禁用 alternate screen |
| `CLAUDE_CODE_FORCE_SYNC_OUTPUT` | env export 与输出策略分支可见 | changelog 中的 `FORCE_SYNC_OUTPUT` 在当前 bundle 应按这个完整 env 名写；作用在输出 flush / sync-output 侧 |
| `DISABLE_UPDATES` | env export 与 update gate 可见 | 属于 update/distribution 入口开关，不应写成 settings merge 里的普通 key |
| `AI_AGENT` | 启动早期 env 判定可见 | 用于 agent identity / nested agent 场景的运行态标识；不是模型选择项 |

这些 key 的共同边界是：**env switch 不等于 settings key**。文档里引用 changelog 线索时，应按当前 bundle 里的实际名称写，尤其是 `CLAUDE_CODE_DISABLE_ALTERNATE_SCREEN`、`CLAUDE_CODE_FORCE_SYNC_OUTPUT`、`CLAUDE_CLIENT_PRESENCE_FILE` 这三项。
...
| `enforceAvailableModels` | default model resolver / model picker | 当 `availableModels` 是非空数组时，把 `Default` 也约束到 allowlist 内 | 未设置或 `availableModels` 为空数组时不生效；它解决的是客户端选择面，不证明服务端组织默认模型如何投放 |
| `modelOverrides` | `BP7()`、`Q28()`、`N3()` | 在 provider 模型表构建时做正向 override；在显示/比较时做反向映射 | 它不是 UI label rewrite，而是 provider model id 映射层 |
| `fallbackModel` | `Kur(...)` retry context、CLI `--fallback-model` | primary model overloaded / unavailable 时按顺序尝试 fallback model；`default` 会展开成默认模型 | CLI `--fallback-model` 优先于 settings；fallback 目标仍受 allowlist 与 provider capability 约束 |
...
### 3.1 version policy 与 managed settings 启动 gate

版本约束 key 不应写成旧的 `requiredMinVersion / requiredMaxVersion`。当前 schema 中可见的是：

| key | 当前语义 | 边界 |
| --- | --- | --- |
| `minimumVersion` | auto-update channel 切换时防止降级到低于该版本 | 是更新策略约束，不等价于启动硬 gate |
| `requiredMinimumVersion` | managed/policy settings 可要求运行版本不得低于该值；低于时启动退出并提示更新 | 只从 managed policy settings enforce，不能写成 user/project/local 任意可控 |
| `requiredMaximumVersion` | managed/policy settings 可要求运行版本不得高于该值；高于时启动退出并提示安装批准版本 | 同样只从 managed policy settings enforce |

证据点在 `package/preprocessed/cli.extracted.bundle.pretty.js:66529-66543`。因此恢复文档里提版本强制策略时，应使用 `requiredMinimumVersion / requiredMaximumVersion` 这两个当前 key 名，并保留 managed-only 边界。
...
| `skills` | `IZ("skills")`、`p_t()`、skill-dir loader | `~/.claude/skills`、项目 `.claude/skills`、动态 skill 目录发现 | managed skill dir、plugin skills、builtin plugin skills、bundled skills | 当前本地证据只显示它切 skill-dir 来源；不是“所有 prompt command 都没了” |
...
| `allowAllClaudeAiMcps` / `disableClaudeAiConnectors` | claude.ai connector resolve 链 | 管理 claude.ai cloud connectors 是否自动获取和连接 | 只约束自动获取的 connector；显式传入的 `claudeai-proxy` 仍走普通 MCP trust / policy 流 |
| `allowManagedMcpServersOnly` | MCP policy filter | 只把 MCP allowlist 读取源收缩到 managed settings | denylist 仍合并所有来源；它不是“只允许 managed server 存在”，而是“只按 managed allowlist 放行” |
...
### 5.1 Agent View / workflow / worktree gate

| 键族 | 主要消费点 | 消费方式 | 关键边界 |
| --- | --- | --- | --- |
| `disableAgentView` | `l3m()` fast-path、fleet gate | 关闭 `claude agents`、`--bg`、`/background` 与 on-demand daemon | 等价环境变量是 `CLAUDE_CODE_DISABLE_AGENT_VIEW=1`；它是启动层 gate，不是单个 TUI dialog 开关 |
| `worktree.bgIsolation` | background session / `EnterWorktree` | 决定 background job 在主 checkout 里是否先被禁止 Edit/Write，直到进入 worktree | `worktree` 是默认隔离；`none` 允许 background job 直接编辑当前 working copy |
| `worktree.baseRef` / `sparsePaths` / `symlinkDirectories` | `--worktree`、`EnterWorktree`、agent isolation | 控制 worktree 分支来源、sparse checkout 与目录 symlink | 这些 key 同时影响手动 `--worktree` 和 agent/workflow 隔离，不应只写成 CLI option |
| `disableWorkflows` / `enableWorkflows` | Workflow tool gate | 关闭或启用 workflows feature | `disableWorkflows` 也可由 `CLAUDE_CODE_DISABLE_WORKFLOWS` 表达；服务端默认是否开启不能由本地 bundle 推出 |
| `workflowKeywordTriggerEnabled` | prompt keyword trigger | 控制 prompt 中的 `ultracode` 关键字是否触发 Workflow tool | 默认描述为 true；这里只能写本地 trigger gate，不写账户级可用性 |
