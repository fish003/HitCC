...
`login/logout` 的本地证据需要分层写：

- 当前 bundle 可见 `mcp login` 的提示面，以及 OAuth token refresh / revoke、metadata discovery、resource metadata 与 mTLS client credential 相关路径。
- `logout` 的 CLI 命令名可见，但完整 dispatcher 行为仍需更细定位；文档不应把它写成已完全复原的 revoke 工作流。
- OAuth / RFC 9728 类 metadata 与 mTLS 属于 MCP transport/auth 边界，不属于普通 `auth login/logout`。

证据点：`package/preprocessed/cli.extracted.bundle.pretty.js:433569-433574`、`115241-115245`、`115430-115436`、`166188`、`333427-334883`、`69703-69755`。
...
连接超时和工具执行超时不是同一个 env：

| env | 作用层 | 边界 |
| --- | --- | --- |
| `MCP_TIMEOUT` | server 连接 / handshake 等待 | 影响 `XC(...)` 连接期超时，缺省约 30000ms |
| `MCP_TOOL_TIMEOUT` | MCP tool 调用执行等待 | 属于工具执行合同，不应和 server connect timeout 混写 |
| `CLAUDE_CODE_MCP_TOOL_IDLE_TIMEOUT` | MCP tool idle 侧 watchdog | 是 tool 运行中 idle 语义，不是连接建链超时 |

另外，当前 bundle 中 `CLAUDE_PROJECT_DIR` 会参与 `${CLAUDE_PROJECT_DIR}` 占位替换。它应理解为 MCP / prompt 侧拿到项目根的一种注入面，而不是 `process.cwd()` 或 MCP root 列表的替代品。
...
### `claude mcp login/logout` 与 OAuth credential 边界

当前命令树显式注册了 `claude mcp login <name>` 和 `claude mcp logout <name>`。`login` 面向 HTTP、SSE 或 claude.ai connector，并支持 `--no-browser`；`logout` 的处理按 server 类型分流：

| server 类型 | `logout` 行为 |
| --- | --- |
| `claudeai-proxy` | 不清本机 credential，提示到 claude.ai 侧断开 connector |
| unsupported transport | 提示该 server 不使用 OAuth，没有本地 credential 可清 |
| anthropic-hosted | 清本地 credential，并输出 hosted connector 对应消息 |
| OAuth server | 清本地 credential / revoke token 后提示已 sign out；如果仍可构造 login 命令，会提示重新 `mcp login` |

证据点在 `package/preprocessed/cli.extracted.bundle.pretty.js:737215-737241`、`737355-737363`。因此这里能写实的是本地 credential 清理、revoke/logging 与提示面；远端 connector 是否已在 claude.ai 侧真正断开，仍要以服务端状态为准。

### `alwaysLoad`、`headersHelper` 与 result size metadata

MCP config schema 当前把部分 key 直接暴露在 server config 层：

| key / metadata | 适用 transport | 当前语义 |
| --- | --- | --- |
| `alwaysLoad` | stdio / sse / http / ws / ide / sdk / claudeai-proxy | server config 为 true，或 tool `_meta["anthropic/alwaysLoad"] === true` 时，对应 MCP tool 可绕过默认 deferred-only 行为进入更积极的加载面 |
| `headersHelper` | sse / http / ws | 作为动态 header helper command / source 的配置位；和静态 `headers` 并列 |
| `_meta["anthropic/maxResultSizeChars"]` | MCP tool result metadata | 若为正数，客户端取该值与全局 ceiling 的较小值作为 `maxResultSizeChars`，并设置 `persistenceThresholdCeiling` |
| `_meta["anthropic/searchHint"]` | MCP tool metadata | 可进入 deferred tool 的 search hint，供 ToolSearch/选择面使用 |

证据点：schema 在 `package/preprocessed/cli.extracted.bundle.pretty.js:63897-64005`，tool metadata consumer 在 `348414-348469`。这说明 MCP server 配置、tool metadata 与 deferred tool 装载策略已经直接耦合。
...
## resource / polling update：`list_changed` consumer 已确认，update tag 仍只到 parser / renderer
...
- `sendResourceListChanged()` 在 generic server / SDK 注册资源、模板资源或更新资源属性时会被调用
- `sendResourceUpdated(...)` 当前只正证到 generic protocol method
- 这些是 **协议 / SDK server 面能力**，不是已经证明 CLI 产品主链会把资源更新写进 transcript 或 prompt

### 2. 连接层支持

- `resources/list_changed` 已有本地 notification handler
- 收到后会：
  - 删除 `O4 / Ude` 的 resource / resourceTemplates cache
  - 在 MCP prompt command 开关打开时同步删除 `jJ / i2o` prompt cache
  - 重新 list resources / templates / commands
  - 刷新本地 connected server snapshot
- 这一支当前本地只看到：
...
- 全 bundle 对 `<mcp-resource-update` / `<mcp-polling-update` 的字面命中，当前只落在：
  - parser 正则
  - renderer 分支
  - transcript sidecar / structured content marker 判断
- `notifications/resources/updated`
  - 当前只正证到 schema / capability assert / generic `sendResourceUpdated(...)`
  - 没看到 CLI 主运行时对它注册 handler
- `sendResourceListChanged()`
  - 当前能看到 generic server / SDK 注册资源与资源模板时的发送点
  - CLI 产品侧对应的是接收 `resources/list_changed` 后刷新 snapshot，而不是生成 update tag
...
- generic MCP SDK 预留了 resources updated/list_changed 的协议能力，并会在 server 端资源注册变更时发送 `list_changed`
...
- **generic MCP 基类具备发送 resource notifications 的协议能力，其中 `list_changed` 有 server 端注册变更发送点**
