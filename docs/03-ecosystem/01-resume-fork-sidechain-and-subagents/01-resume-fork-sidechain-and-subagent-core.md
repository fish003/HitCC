...
当前 `2.1.197` 里这条线应按 `zN(...) / oRf(...)` 主执行骨架、`_3(...)` 非主线程入口、`forkContextMessages` 和 sidechain transcript adapter 一起理解。  
sidechain transcript 与主线程 transcript 分开写；subagent 走 `subagents/agent-<id>.jsonl`。旧文档里的 `CC` 只适合作为旧压缩名线索，不再作为当前函数名引用。
...
1. implicit fork
   ...
   - 语义不是“fresh agent”，而是：
     ...
     - 但 request-level `userContext / systemContext` 不是整包照搬，而是仍按 `_3(...)` / `zN(...)` 相关执行入口的默认装配再做裁剪/override
...
- 一旦发现已经被后台化：
  ...
  - 以 `isAsync: true` 重新接一条后台 agent 执行流
