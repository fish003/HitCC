...
- [07-tui-system/06-tool-permission-dispatch.md](./07-tui-system/06-tool-permission-dispatch.md)：覆盖 tool permission overlay、tool-specific dialog registry、path-aware 审批壳和 plan/skill/AskUserQuestion 等专用审批组件。
...
- [07-tui-system/08-tool-result-renderers.md](./07-tui-system/08-tool-result-renderers.md)：覆盖 `zpl(...)` 与 `toolUseResult` sidecar、lookup/progress/rejection/error 特判，以及高价值结果渲染器家族。
