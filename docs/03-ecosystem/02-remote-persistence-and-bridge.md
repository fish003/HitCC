...
Chrome/native-host/computer-use 在入口层也属于 fast-path，但它们是独立旁路模式，不归入 bridge / remote-control 工作流矩阵。
...
### host integration surface 的本地边界

当前 bundle 能证明的是本地 CLI/terminal/SDK 对 host 的识别与适配面，不应扩写成外部插件内部实现。

本地可见 host surface 至少包括：

- terminal / IDE 识别：
  - `VSCODE_GIT_ASKPASS_MAIN`、`CURSOR_TRACE_ID`、`__CFBundleIdentifier`、`TERMINAL_EMULATOR=JetBrains-JediTerm`、`TERM_PROGRAM`、`WSL_DISTRO_NAME` 等环境信号会影响 terminal / IDE 名称归类
  - `windsurf` 分支当前还会覆盖 Devin Desktop / Windsurf / devin.app 一组路径和 bundle id
- `/terminal-setup`：
  - VS Code、Cursor、Devin Desktop、Zed、Alacritty、Apple Terminal 等路径都有本地配置写入或提示分支
  - 对 VS Code / Cursor / Devin Desktop，会尝试写 terminal Shift+Enter keybinding，并可调整 `terminal.integrated.mouseWheelScrollSensitivity` 和 `terminal.integrated.gpuAcceleration`
  - 远端 SSH 场景会提示必须在本机 host 配置，不会把远端服务器当作本机 IDE 配置目录
- JetBrains：
  - `TERMINAL_EMULATOR=JetBrains-JediTerm` 会进入专门检测；TUI 输出策略中也有 JetBrains terminal 特例
- Remote Control / host UI：
  - host UI 可通过 initialize / notification 面接收当前模型、disabled models、feedback survey 配置、`open_message_selector` 等 host-facing event
  - 这证明 CLI 为 VS Code webview、Claude Desktop、Remote Control 等 host 暴露状态面；外部 host 如何渲染这些状态仍不在本地 bundle 证明范围内

`CLAUDE_CLIENT_PRESENCE_FILE` 是另一条容易误读的 host lifecycle 信号。当前本地链路是：

- 若 env 未设置，presence marker 不生效
- 若设置的是相对路径且不满足安全路径条件，本地直接视为无效
- 若文件存在，才认为 client presence marker 可用
- session presence 的网络上报走 `/v1/code/sessions/{sessionId}/client/presence`，负载至少包含 `client_id`，并可带 `connected_at` 或 `clear`

所以 `CLAUDE_CLIENT_PRESENCE_FILE` 不是“任意 host 连接即在线”的布尔值，而是本地 marker file 与 remote session presence pulse 共同构成的 host lifecycle 辅助信号。
