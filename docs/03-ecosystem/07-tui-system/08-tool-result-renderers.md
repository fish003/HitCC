...
- 专门拆开 `user.tool_result -> zpl(...)` 这一支，不再把它混在 subtype 总表里。
...
`zpl(...)` 先用 `tool_use_id` 去 `lookups.toolUseByToolUseID` 找回原始 tool 与 tool_use。找不到就直接返回 `null`。
...
错误态统一走 `a7n(...)`，但里面不是单一 fallback，而是继续拆成几类特殊字符串：
...
`zpl(...)`、assistant tool-use renderer 与 `mpl(...)` 都依赖这组索引。
...
`Read.renderToolResultMessage = SPl(...)` 已经能稳定分成五类：
...
`Bash.renderToolResultMessage = rxl(...)`、`PowerShell.renderToolResultMessage = tIl(...)` 共享 shell 输出摘要策略。  
它当前至少会处理：
...
`Edit.renderToolResultMessage = Pxl(...)`：
...
`Write.renderToolResultMessage = Zsl(...)`：
...
`NotebookEdit.renderToolResultMessage = Iil(...)` 当前很明确：
...
- `Grep.renderToolResultMessage = cil(...)`
...
`WebFetch.renderToolResultMessage = rml(...)`：

- 默认只显示：
  - `Received <size> (<code> <codeText>)`
- verbose 才把 `result` 正文带上

`WebSearch.renderToolResultMessage = QAl(...)`：
...
低频工具不需要逐个做复杂卡片，但当前 bundle 已经能看出几类稳定策略：

- MCP resource / generic MCP
  - `ega / aFn / oRa / SRa` 这组 renderer 基本按 JSON / text 摘要显示，并通过 `isResultTruncated(...)` 把过宽或过长内容交给展开路径。
- plan / skill
  - `ExitPlanMode`、`EnterPlanMode` 和 `SkillTool` 的结果 renderer 更偏状态提示；核心语义已经在审批 UI 和执行器里处理，结果行不承担完整正文展示。
- agent / background task / workflow
  - `tfl(...)` 会把 async / remote / completed 分成 “Backgrounded agent / Cloud agent launched / Done(...)” 等状态行；transcript 模式下才补 prompt、progress 和 response。
  - workflow、monitor、background control 等工具的 renderer 也以状态摘要和可展开进度为主。
- worktree / scheduled task / user notification / file delivery
  - 多数只显示 action 已完成、对象 ID、路径或数量；正文继续依赖 transcript content 或 attachment。

因此低频结果的候选实现不需要为每个 tool 手写等价 UI。更稳的 registry 形态是：

```text
tool_result sidecar
-> tool.renderToolResultMessage(...)
-> optional isResultTruncated / extractSearchText
-> generic JSON/text/status fallback
```

这能覆盖当前低频工具，同时保留 bundle 外工具和 deferred tool 的扩展位。

这里的 fallback 要按两层理解：

- 当前本地 TUI 的 `Fpl(...)` 正常结果分支会先做 `outputSchema.safeParse(...)`，再调用工具对象自己的 `renderToolResultMessage(...)`；如果 schema 校验失败、没有 `toolUseResult`、工具缺失或 renderer 返回空值，这一行会直接不显示普通结果块。
- 因此 `generic JSON/text/status fallback` 更适合写成重写时的 registry 默认项，而不是声称原版在 `Fpl(...)` 后面还有一个全局兜底 renderer。
- 当前能作为这个默认项依据的，是一批低频工具已经各自实现了相似的 JSON / text / status 摘要 renderer，例如 MCP resource、generic MCP、workflow/background、file delivery、scheduled task 和 user notification 一类工具。
...
ToolResultRendererRegistry
  ...
  -> default JSON/text/status renderer
  -> null
```

但兼容原版行为时要保留：

- lookup 不到 `tool_use_id` 对应工具时返回空。
- schema parse 失败时返回空。
- tool 自身声明 transparent wrapper 时返回空。
- 默认 fallback 只用于 bundle 外、deferred 或新接入工具；已知 builtin tool 仍以自己的 `renderToolResultMessage(...)` 为准。
...
- `zpl(...) -> Fpl(...)` 正常结果分支并不直接渲染 `tool_result.content`，而是回查 tool metadata 与结构化结果。
- 本地 `Fpl(...)` 没有额外的全局 JSON fallback；低频 generic fallback 是重写 registry 的默认项，依据来自低频 concrete renderer 的共同形态。
...
- `Fpl(...)` 里 `Allowed by auto mode classifier` 旁路提示所依赖的局部状态来源还不够完整，本地只看到 classifier approval store 的读取点。
...
- `package/preprocessed/cli.extracted.bundle.pretty.js`
  ...
  - `Fpl(...)`
  - `zpl(...)`
  - shell renderer `rxl / tIl`
  - file result renderer `Zsl / Pxl`
  - `NotebookEdit` 的 `Iil(...)`
  - `Grep / Glob` 的 `cil / mil`
  - `WebFetch / WebSearch` 的 `rml / QAl`
  - MCP / low-frequency renderer `ega / aFn / oRa / SRa / tfl`
