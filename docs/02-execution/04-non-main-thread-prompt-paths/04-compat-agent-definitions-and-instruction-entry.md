...
1. **`_3(...)` fresh build**
   ...
   - 那它会随 `userContext.claudeMd` 进入 `otr(...)`
2. **`vk(...)` snapshot reuse**
   - 不重新扫描 compat
...
- 当前 `2.1.197` 抽取 bundle 里没有再命中旧文档记录的 `verification_agent` query source
...
- 还可以再把“没有完整本地 wiring”写成一组**反证闭环**：
  ...
  - 旧 verifier prompt 文本
    - `You are a verification specialist.` / `VERDICT: PASS` 当前本地抽取 bundle 没有命中
    - 没有看到 `getSystemPrompt` 或别的本地 consumer
  - `subagent_type="verification"`
    - 只作为 Task 工具说明里的示例词义出现，不是 launch path
...
- `currentDate / gitStatus / perforceMode / claudeMd` 的本地后续命中也已基本穷尽，没有再看到 `_3(...)` 之外的专门裁剪点
...
`Qv()` 会把当前工作目录一直向上推到文件系统根，再按 `root -> cwd` 顺序依次加载。  
